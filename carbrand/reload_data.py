...
def save_carbrand_data():
    ...
    cursor = db.cursor()
    with open('../car_new.json', 'r') as file:
        ...
        for car in list_car:
            ...
            sql = "INSERT INTO carbrand_carbrand(NAME_CN, IMG, CARBRAND_ID, RANKING, COUNTRY_ID) VALUES ('%s','%s','%d','%d','%d') " \
                  % (name_cn, img, carbrand_id, ranking, country_id)
            print sql
            try:
                cursor.execute(sql)
                db.commit()
